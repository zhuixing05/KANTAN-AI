...
function tryKantanaiServerFallback(modelId?: string): MatchedProvider | null {
  ...
  console.log('[ClaudeSettings] kantanai-server fallback activated:', { baseURL, modelId: effectiveModelId, supportsImage: cachedMeta?.supportsImage });
  return {
...
function resolveMatchedProvider(appConfig: AppConfig): { matched: MatchedProvider | null; error?: string } {
  ...
  if (!modelId) {
    ...
    if (!fallback) {
      const serverFallback = tryKantanaiServerFallback(configuredModelId);
  ...
  // Handle kantanai-server provider: dynamically construct from auth tokens
  if (preferredProviderName === ProviderName.KantanaiServer) {
    const serverMatch = tryKantanaiServerFallback(modelId);
    if (serverMatch) {
  ...
  if (!providerEntry) {
    ...
    if (fallback) {
      ...
      const serverFallback = tryKantanaiServerFallback(modelId);
      if (serverFallback) return { matched: serverFallback };
...
export function resolveCurrentApiConfig(target: OpenAICompatProxyTarget = 'local'): ApiConfigResolution {
  ...
  const effectiveApiKey = resolvedApiKey
    || (!providerRequiresApiKey(matched.providerName) ? 'sk-kantanai-local' : '');
  ...
  return {
    config: {
      apiKey: resolvedApiKey || 'kantanai-openai-compat',
...
export function resolveAllProviderApiKeys(): Record<string, string> {
  ...
  // kantanai-server token is now managed by the token proxy
  ...
  for (const [providerName, providerConfig] of Object.entries(appConfig.providers)) {
    ...
    result[envName] = apiKey || 'sk-kantanai-local';
  }
...
export function resolveAllEnabledProviderConfigs(): ProviderRawConfig[] {
  ...
  for (const [providerName, providerConfig] of Object.entries(appConfig.providers)) {
    if (!providerConfig?.enabled) continue;
    if (providerName === ProviderName.KantanaiServer) continue;
    ...
    result.push({
      ...
      apiKey: apiKey || 'sk-kantanai-local',
