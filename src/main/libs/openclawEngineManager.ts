...
export class OpenClawEngineManager extends EventEmitter {
  ...
  private async doStartGateway(): Promise<OpenClawEngineStatus> {
    ...
    const env: NodeJS.ProcessEnv = {
      ...
      KANTANAI_SKILLS_ROOT: skillsRoot,
      ...
      KANTANAI_ELECTRON_PATH: electronNodeRuntimePath.replace(/\\/g, '/'),
      KANTANAI_OPENCLAW_ENTRY: openclawEntry.replace(/\\/g, '/'),
    ...
    // find the KANTAN AI-managed Python instead of the Windows Store stub.
  ...
  private ensureBundledCliShims(): string | null {
    ...
    const shellWrapper = [
      '#!/usr/bin/env bash',
      'if [ -z "${KANTANAI_OPENCLAW_ENTRY:-}" ]; then',
      '  echo "KANTANAI_OPENCLAW_ENTRY is not set" >&2',
      '  exit 127',
      'fi',
      'if [ -n "${KANTANAI_ELECTRON_PATH:-}" ]; then',
      '  exec env ELECTRON_RUN_AS_NODE=1 "${KANTANAI_ELECTRON_PATH}" "${KANTANAI_OPENCLAW_ENTRY}" "$@"',
      'fi',
      'if command -v node >/dev/null 2>&1; then',
      '  exec node "${KANTANAI_OPENCLAW_ENTRY}" "$@"',
      'fi',
      'echo "Neither KANTANAI_ELECTRON_PATH nor node is available for OpenClaw CLI." >&2',
      'exit 127',
      '',
    ].join('\n');
    const windowsWrapper = [
      '@echo off',
      'if "%KANTANAI_OPENCLAW_ENTRY%"=="" (',
      '  echo KANTANAI_OPENCLAW_ENTRY is not set 1>&2',
      '  exit /b 127',
      ')',
      'if not "%KANTANAI_ELECTRON_PATH%"=="" (',
      '  set ELECTRON_RUN_AS_NODE=1',
      '  "%KANTANAI_ELECTRON_PATH%" "%KANTANAI_OPENCLAW_ENTRY%" %*',
      '  exit /b %ERRORLEVEL%',
      ')',
      'node "%KANTANAI_OPENCLAW_ENTRY%" %*',
      '',
    ].join('\r\n');
