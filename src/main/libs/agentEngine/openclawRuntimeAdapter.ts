...
export class OpenClawRuntimeAdapter extends EventEmitter implements CoworkRuntime {
   ...
   * so KANTAN AI can recover even when the gateway fails to deliver the abort event.
   */
   ...
   * for a KANTAN AI-initiated session.
   */
  ...
  private buildSystemPromptPrefix(systemPrompt: string): string {
    return [
      '[KANTAN AI system instructions]',
      'Apply the instructions below as the highest-priority guidance for this session.',
      'If earlier KANTAN AI system instructions exist, replace them with this version.',
      systemPrompt,
    ].join('\n');
  }
  ...
  private buildBridgePrefix(messages: CoworkMessage[], currentPrompt: string): string {
    ...
    return [
      '[Context bridge from previous KANTAN AI conversation]',
   ...
   * gateway rather than the KANTAN AI UI.
   *
