...
const MANAGED_WEB_SEARCH_POLICY_PROMPT = [
  ...
  '- Only use the KANTAN AI `web-search` skill when local command execution is available. Native channel sessions may deny `exec`, so prefer `browser` or `web_fetch` there.',
  ...
  '',
  'Do not claim you searched the web unless you actually used `browser`, `web_fetch`, or the KANTAN AI `web-search` skill.',
].join('\n');
...
const PROVIDER_REGISTRY: Record<string, ProviderDescriptor> = {
  [ProviderName.KantanaiServer]: {
    providerId: OpenClawProviderId.KantanaiServer,
...
export class OpenClawConfigSync {
  ...
  sync(reason: string): OpenClawConfigSyncResult {
    ...
    const proxyPort = getOpenClawTokenProxyPort();
    if (proxyPort && !allProvidersMap[ProviderName.KantanaiServer]) {
      ...
      const firstServerSel = buildProviderSelection({
        ...
        apiType: 'openai',
        providerName: ProviderName.KantanaiServer,
        supportsImage: serverModels[0]?.supportsImage,
      });
      const kantanaiProviderConfig = { ...firstServerSel.providerConfig, models: [] as typeof firstServerSel.providerConfig.models };
      for (const sm of serverModels) {
        kantanaiProviderConfig.models.push({
          id: sm.modelId,
          name: sm.modelId,
      ...
      if (kantanaiProviderConfig.models.length === 0) {
        kantanaiProviderConfig.models.push(firstServerSel.providerConfig.models[0]);
      }
      allProvidersMap[OpenClawProviderId.KantanaiServer] = kantanaiProviderConfig;
    }
    ...
    if (hasMcpBridgePlugin && mcpBridgeCfg && mcpBridgeCfg.tools.length > 0 && managedConfig.plugins) {
      ...
      entries['mcp-bridge'] = {
        ...
        config: {
          ...
          secret: '${KANTAN_MCP_BRIDGE_SECRET}',
    ...
    if (tgConfig?.enabled && tgConfig.botToken) {
      ...
      if (tgConfig.webhookUrl) {
        ...
        if (tgConfig.webhookSecret) {
          telegramChannel.webhookSecret = '${KANTAN_TG_WEBHOOK_SECRET}';
        }
      }
    ...
    if (feishuConfig?.enabled && feishuConfig.appId) {
      const feishuChannel: Record<string, unknown> = {
        ...
        appSecret: '${KANTAN_FEISHU_APP_SECRET}',
    ...
    if (dingTalkConfig?.enabled && dingTalkConfig.clientId) {
      ...
      const dingtalkChannel: Record<string, unknown> = {
        ...
        clientSecret: '${KANTAN_DINGTALK_CLIENT_SECRET}',
        ...
        ...(gatewayToken ? { gatewayToken: '${KANTAN_DINGTALK_GW_TOKEN}' } : {}),
      };
    ...
    if (qqConfig?.enabled && qqConfig.appId) {
      const qqChannel: Record<string, unknown> = {
        ...
        clientSecret: '${KANTAN_QQ_CLIENT_SECRET}',
    ...
    if (popoConfig?.enabled && popoConfig.appKey) {
      ...
      const popoChannel: Record<string, unknown> = {
        ...
        appSecret: '${KANTAN_POPO_APP_SECRET}',
      ...
      // Webhook-only fields
      if (!isWebSocket) {
        popoChannel.token = '${KANTAN_POPO_TOKEN}';
  ...
  collectSecretEnvVars(): Record<string, string> {
    ...
    for (const [envSuffix, apiKey] of Object.entries(allApiKeys)) {
      env[`KANTAN_APIKEY_${envSuffix}`] = apiKey;
    }
    // Legacy fallback: keep KANTAN_PROVIDER_API_KEY set to a stable value so stale
    ...
    env.KANTAN_PROVIDER_API_KEY = 'legacy-unused';

    // MCP Bridge Secret — always set so stale openclaw.json with
    // ${KANTAN_MCP_BRIDGE_SECRET} placeholder doesn't crash the gateway.
    const mcpBridgeCfg = this.getMcpBridgeConfig?.();
    env.KANTAN_MCP_BRIDGE_SECRET = mcpBridgeCfg?.secret || 'unconfigured';

    // Telegram
    const tgConfig = this.getTelegramOpenClawConfig?.();
    if (tgConfig?.enabled && tgConfig.botToken) {
      env.KANTAN_TG_BOT_TOKEN = tgConfig.botToken;
      if (tgConfig.webhookSecret) {
        env.KANTAN_TG_WEBHOOK_SECRET = tgConfig.webhookSecret;
      }
    }
    ...
    const dcConfig = this.getDiscordOpenClawConfig?.();
    if (dcConfig?.enabled && dcConfig.botToken) {
      env.KANTAN_DC_BOT_TOKEN = dcConfig.botToken;
    }

    // Feishu
    const feishuConfig = this.getFeishuConfig();
    if (feishuConfig?.enabled && feishuConfig.appSecret) {
      env.KANTAN_FEISHU_APP_SECRET = feishuConfig.appSecret;
    }

    // DingTalk
    const dingTalkConfig = this.getDingTalkConfig();
    if (dingTalkConfig?.enabled && dingTalkConfig.clientSecret) {
      env.KANTAN_DINGTALK_CLIENT_SECRET = dingTalkConfig.clientSecret;
    }
    const gatewayToken = this.engineManager.getGatewayToken();
    if (gatewayToken) {
      env.KANTAN_DINGTALK_GW_TOKEN = gatewayToken;
    }

    // QQ
    const qqConfig = this.getQQConfig();
    if (qqConfig?.enabled && qqConfig.appSecret) {
      env.KANTAN_QQ_CLIENT_SECRET = qqConfig.appSecret;
    }

    // WeCom
    const wecomConfig = this.getWecomConfig();
    if (wecomConfig?.enabled && wecomConfig.secret) {
      env.KANTAN_WECOM_SECRET = wecomConfig.secret;
    }

    // POPO
    const popoConfig = this.getPopoConfig();
    if (popoConfig?.enabled && popoConfig.appSecret) {
      env.KANTAN_POPO_APP_SECRET = popoConfig.appSecret;
    }
    if (popoConfig?.enabled && popoConfig.token) {
      env.KANTAN_POPO_TOKEN = popoConfig.token;
    } else if (popoConfig?.enabled) {
      // Provide non-empty fallback so stale openclaw.json files that still
      // contain ${KANTAN_POPO_TOKEN} from a previous webhook config
      // don't crash the gateway with MissingEnvVarError.
      env.KANTAN_POPO_TOKEN = 'unconfigured';
    }

    // NIM
    const nimConfig = this.getNimConfig();
    if (nimConfig?.enabled && nimConfig.token) {
      env.KANTAN_NIM_TOKEN = nimConfig.token;
    }
  ...
  private syncManagedSessionStore(selection: OpenClawProviderSelection): boolean {
    const shouldMigrateManagedModelRefs = !(
      selection.providerId === 'kantan' && selection.sessionModelId === selection.legacyModelId
    );
    ...
    for (const [sessionKey, rawEntry] of Object.entries(sessionStore)) {
      ...
      if (!shouldMigrateManagedModelRefs || !(/^agent:[^:]+:kantanai:/.test(sessionKey))) {
        continue;
      }
      ...
      if (entryProvider !== 'kantan' || entryModel !== selection.legacyModelId) {
        continue;
      }
      ...
      if (systemPromptReport && typeof systemPromptReport === 'object') {
        ...
        if (typeof report.provider === 'string' && report.provider.trim() === 'kantan') {
  ...
  /**
   * Resolve the KANTAN AI SKILLs installation directory for OpenClaw's
   * `skills.load.extraDirs` configuration.
   *
   * Cross-platform paths (via Electron app.getPath('userData')):
   *   macOS:   ~/Library/Application Support/KANTAN AI/SKILLs
   *   Windows: %APPDATA%/KANTAN AI/SKILLs
   *   Linux:   ~/.config/KANTAN AI/SKILLs
   */
  ...
  /**
   * Build per-skill `enabled` overrides from the KANTAN AI SkillManager state,
   * so that skills disabled in the KANTAN AI UI are also hidden from OpenClaw.
   */
