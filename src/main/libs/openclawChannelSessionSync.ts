 ...
 * to local Cowork sessions so that conversations are visible in the KANTAN AI UI.
 */
...
const KANTANAI_SESSION_PREFIX = 'kantanai:';
...
export function buildManagedSessionKey(
  ...
  return `agent:${normalizedAgentId}:kantanai:${normalizedSessionId}`;
}
...
export function parseManagedSessionKey(sessionKey: string | undefined | null): ManagedSessionKey | null {
  ...
  if (raw.startsWith(KANTANAI_SESSION_PREFIX)) {
    const sessionId = raw.slice(KANTANAI_SESSION_PREFIX.length).trim();
  ...
  if (parts.length < 4 || parts[0] !== 'agent' || parts[2] !== 'kantanai') {
    return null;
  }
...
export class OpenClawChannelSessionSync {
  ...
  resolveOrCreateSession(sessionKey: string): string | null {
    // 1. Skip KANTAN AI-originated sessions
    if (isManagedSessionKey(sessionKey)) {
      console.log('[ChannelSessionSync] skipped: KANTAN AI-originated session');
      return null;
    }
