...
const TASK_WORKSPACE_CONTAINER_DIR = '.kantanai-tasks';
...
export class CoworkRunner extends EventEmitter {
  ...
  private async runClaudeCodeLocal(
    ...
    coworkLog('INFO', 'runClaudeCodeLocal', `MCP env: KANTANAI_ELECTRON_PATH=${envVars.KANTANAI_ELECTRON_PATH || '(not set)'}`);
    ...
    coworkLog('INFO', 'runClaudeCodeLocal', `MCP env: KANTANAI_NPM_BIN_DIR=${envVars.KANTANAI_NPM_BIN_DIR || '(not set)'}`);
    ...
    if (process.platform === 'win32' && !envVars.CLAUDE_CODE_GIT_BASH_PATH) {
      const bashResolutionDiagnostic = typeof envVars.KANTANAI_GIT_BASH_RESOLUTION_ERROR === 'string'
        ? envVars.KANTANAI_GIT_BASH_RESOLUTION_ERROR.trim()
        : '';
    ...
    if (app.isPackaged) {
      ...
      options.spawnClaudeCodeProcess = (spawnOptions: {
        ...
        const useElectronShim =
          ...
          || isPackagedDarwin
          || spawnOptions.env?.KANTANAI_NODE_SHIM_ACTIVE === '1';
        const spawnEnv: NodeJS.ProcessEnv = {
          ...(spawnOptions.env ?? {}),
          ELECTRON_RUN_AS_NODE: '1',
        };
        if (useElectronShim) {
          spawnEnv.KANTANAI_ELECTRON_PATH = spawnOptions.env?.KANTANAI_ELECTRON_PATH || electronNodeRuntimePath;
        } else {
          delete spawnEnv.KANTANAI_ELECTRON_PATH;
        }
        ...
        if (process.platform === 'win32' && isNodeLikeCommand) {
          ...
          spawnEnv.KANTANAI_ELECTRON_PATH = electronNodeRuntimePath;
              ...
              case 'stdio':
                {
                  ...
                  if (process.platform === 'win32' && app.isPackaged && effectiveStdioCommand) {
                    ...
                    const npmBinDir = envVars.KANTANAI_NPM_BIN_DIR;
                    ...
                    const withElectronNodeEnv = (base: Record<string, string> | undefined): Record<string, string> => ({
                      ...(base || {}),
                      ELECTRON_RUN_AS_NODE: '1',
                      KANTANAI_ELECTRON_PATH: electronNodeRuntimePath,
                    });
