...
export function ensureElectronNodeShim(electronPath: string, npmBinDir?: string): string | null {
  try {
    ...
    const nodeShContent = [
      '#!/usr/bin/env bash',
      'if [ -z "${KANTANAI_ELECTRON_PATH:-}" ]; then',
      '  echo "KANTANAI_ELECTRON_PATH is not set" >&2',
      '  exit 127',
      'fi',
      'exec env ELECTRON_RUN_AS_NODE=1 "${KANTANAI_ELECTRON_PATH}" "$@"',
      '',
    ].join('\n');
    ...
    if (process.platform === 'win32') {
      ...
      const nodeCmdContent = [
        '@echo off',
        'if "%KANTANAI_ELECTRON_PATH%"=="" (',
        '  echo KANTANAI_ELECTRON_PATH is not set 1>&2',
        '  exit /b 127',
        ')',
        'set ELECTRON_RUN_AS_NODE=1',
        '"%KANTANAI_ELECTRON_PATH%" %*',
        '',
      ].join('\r\n');
    ...
    if (npmBinDir && existsSync(npmBinDir)) {
      ...
      if (existsSync(npxCliJs)) {
        ...
        // npx.cmd for Windows — uses %KANTANAI_NPM_BIN_DIR% env var to avoid
        // hardcoding paths that may contain non-ASCII chars (breaks GBK cmd.exe).
        if (process.platform === 'win32') {
          const npxCmd = join(shimDir, 'npx.cmd');
          const npxCmdContent = [
            '@echo off',
            '"%~dp0node.cmd" "%KANTANAI_NPM_BIN_DIR%\\npx-cli.js" %*',
            '',
          ].join('\r\n');
          writeFileSync(npxCmd, npxCmdContent, 'utf8');
          coworkLog('INFO', 'resolveNodeShim', `Created npx.cmd shim: ${npxCmd} (using env var KANTANAI_NPM_BIN_DIR)`);
        }
      } else {
      ...
      if (existsSync(npmCliJs)) {
        ...
        // npm.cmd for Windows — uses %KANTANAI_NPM_BIN_DIR% env var to avoid
        // hardcoding paths that may contain non-ASCII chars (breaks GBK cmd.exe).
        if (process.platform === 'win32') {
          ...
          coworkLog('INFO', 'resolveNodeShim', `Created npm.cmd shim: ${npmCmd} (using env var KANTANAI_NPM_BIN_DIR)`);
        }
      } else {
...
function resolveWindowsGitBashPath(): string | null {
  ...
  // 2. Bundled PortableGit (preferred default in KANTAN AI package)
...
function ensureWindowsBashUtf8InitScript(): string | null {
  try {
    ...
    const content = [
      '#!/usr/bin/env bash',
      '# Auto-generated by KANTAN AI – switch Windows console code page to UTF-8',
...
function applyPackagedEnvOverrides(env: Record<string, string | undefined>): void {
  ...
  if (app.isPackaged && !env.KANTANAI_ELECTRON_PATH) {
    env.KANTANAI_ELECTRON_PATH = electronNodeRuntimePath;
  }
  ...
  if (process.platform === 'win32') {
    ...
    if (configuredBashPath && bashPath === configuredBashPath) {
      ...
      if (!configuredHealth.ok) {
        ...
        if (fallbackPath && fallbackPath !== configuredBashPath) {
          ...
          env.KANTANAI_GIT_BASH_RESOLUTION_ERROR = diagnostic;
          coworkLog('WARN', 'resolveGitBash', diagnostic);
          bashPath = null;
        }
      }
    ...
    if (bashPath) {
      ...
      delete env.KANTANAI_GIT_BASH_RESOLUTION_ERROR;
      ...
      env.KANTANAI_GIT_BASH_RESOLUTION_ERROR = truncateDiagnostic(diagnostic);
    }
  ...
  env.KANTANAI_NPM_BIN_DIR = npmBinDir;
  ...
  if (shouldInjectShim) {
    ...
    if (shimDir) {
      ...
      env.KANTANAI_NODE_SHIM_ACTIVE = '1';
    ...
    delete env.KANTANAI_NODE_SHIM_ACTIVE;
...
function verifyNodeEnvironment(env: Record<string, string | undefined>): void {
        ...
        if (tool === 'node' && resolvedForExec) {
          try {
            ...
            if (process.platform === 'win32' && /\.cmd$/i.test(resolvedForExec)) {
              execTarget = env.KANTANAI_ELECTRON_PATH || process.execPath;
            }
  ...
  coworkLog('INFO', tag, `KANTANAI_ELECTRON_PATH=${env.KANTANAI_ELECTRON_PATH || '(not set)'}`);
  coworkLog('INFO', tag, `KANTANAI_NPM_BIN_DIR=${env.KANTANAI_NPM_BIN_DIR || '(not set)'}`);
...
export function getSkillsRoot(): string {
  ...
  const envRoots = [process.env.KANTANAI_SKILLS_ROOT, process.env.SKILLS_ROOT]
...
export async function getEnhancedEnv(target: OpenAICompatProxyTarget = 'local'): Promise<Record<string, string | undefined>> {
  ...
  env.KANTANAI_SKILLS_ROOT = skillsRoot; // Alternative name for clarity
  if (process.platform === 'win32' || env.KANTANAI_NODE_SHIM_ACTIVE === '1') {
    env.KANTANAI_ELECTRON_PATH = getElectronNodeRuntimePath().replace(/\\/g, '/');
  } else {
    delete env.KANTANAI_ELECTRON_PATH;
  }
