...
async function handleRequest(
  ...
  // Inject session_id and user_message for kantanai-server logging only.
  // Strict providers (e.g. Gemini) reject unknown payload fields.
  if (upstreamConfig.provider === 'kantanai-server') {
  ...
  if (!upstreamResponse.ok) {
    // 401/403 from kantanai-server likely means the JWT accessToken expired.
