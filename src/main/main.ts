...
const resolveInlineAttachmentDir = (cwd?: string): string => {
  ...
  return path.join(app.getPath('temp'), 'kantanai', 'attachments');
};
...
const buildLogExportFileName = (): string => {
  ...
  return `kantanai-logs-${datePart}-${timePart}.zip`;
};
...
const disableGpu =
  process.env.KANTANAI_DISABLE_GPU === '1' ||
  process.env.KANTANAI_DISABLE_GPU === 'true' ||
...
const bindCoworkRuntimeForwarder = (): void => {
  ...
  runtime.on('complete', (sessionId: string, claudeSessionId: string | null) => {
    ...
    try {
      ...
      if (apiConfig.providerMetadata?.providerName === 'kantanai-server') {
        const windows = BrowserWindow.getAllWindows();
        windows.forEach((win) => {
          if (win.isDestroyed()) return;
...
if (!gotTheLock) {
  ...
  app.on('second-instance', (_event, commandLine, workingDirectory) => {
    ...
    const deepLink = commandLine.find(arg => arg.startsWith('kantanai://'));
    if (deepLink) {
  ...
  ipcMain.handle('mcp:fetchMarketplace', async () => {
    const url = app.isPackaged
      ? 'https://api-overmind.youdao.com/openapi/get/luna/hardware/kantanai/prod/mcp-marketplace'
      : 'https://api-overmind.youdao.com/openapi/get/luna/hardware/kantanai/test/mcp-marketplace';
    try {
  ...
  const initApp = async () => {
    ...
    const defaultProjectDir = path.join(os.homedir(), 'kantanai', 'project');
    ...
    // Inject auth getters for kantanai-server provider routing
    ...
    // kantanai-server provider can use the proxy URL in its config.
    try {
    ...
    const coldStartDeepLink = process.argv.find(arg => arg.startsWith('kantanai://'));
