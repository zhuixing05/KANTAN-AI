...
test('infer: managed key without IM channel -> cowork origin + ui_session binding', () => {
  const result = inferOriginAndBinding(
    makeTask({ sessionKey: 'agent:main:kantanai:sess-001', delivery: { mode: DeliveryMode.None } })
  );
...
test('infer: managed key with IM announce channel -> im origin + im_session binding', () => {
  const result = inferOriginAndBinding(
    makeTask({ sessionKey: 'agent:main:kantanai:sess-002', delivery: { mode: DeliveryMode.Announce, channel: 'telegram' } })
  );
...
test('infer: non-main agentId managed key -> cowork origin', () => {
  const result = inferOriginAndBinding(
    makeTask({ sessionKey: 'agent:secondary:kantanai:sess-003', delivery: { mode: DeliveryMode.None } })
  );
...
test('infer: managed key with channel=last -> cowork origin (last is not an IM platform)', () => {
  const result = inferOriginAndBinding(
    makeTask({ sessionKey: 'agent:main:kantanai:sess-004', delivery: { mode: DeliveryMode.Announce, channel: DeliveryChannel.Last } })
  );
...
test('infer: sessionKey with whitespace is trimmed before parsing', () => {
  const result = inferOriginAndBinding(
    makeTask({ sessionKey: '  agent:main:kantanai:sess-trimmed  ' })
  );
  expect(result.origin.kind).toBe(OriginKind.Cowork);
...
test('infer: pure function - same input, same output', () => {
  const task = makeTask({ sessionKey: 'agent:main:kantanai:sess-stable' });
