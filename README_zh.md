...
**Kantan AI** 是由[网易有道LobsterAI](https://www.youdao.com/)二次开发的全场景个人助理 Agent。它 7×24 小时待命，能够帮你完成日常办公中的各类事务 —— 数据分析、制作 PPT、生成视频、撰写文档、搜索信息、收发邮件、定时任务，以及更多。

Kantan AI 的核心是 **Cowork 模式**，它能在本地或沙箱环境中执行工具、操作文件、运行命令，一切都在你的监督下自主完成。此外，Kantan AI 支持通过钉钉、飞书、Telegram、Discord 等 IM 平台远程触发，让你在手机上也能随时指挥 Agent 工作。
...
Kantan AI 可以使用 [OpenClaw](https://github.com/openclaw/openclaw) 作为 Agent 引擎。
...
如果脚本运行失败，请手动执行以下命令：
如果自动脚本遇到权限问题，你可以手动在终端中逐步执行：

# 1. 进入目录
cd /home/kantan/KANTAN-AI

# 2. 安装编译工具 (输入密码 123456)
sudo apt-get update
sudo apt-get install -y build-essential python3 python3-pip python3-distutils libtool nasm autoconf automake
...
该运行时以解释器为主，不预装 Kantan AI 技能所需的 Python 三方包；相关依赖可在运行时按需安装。
默认情况下，如果未提供预构建压缩包，打包脚本会直接从 python.org 下载官方 embeddable Python 运行时。
离线或无法联网的构建场景，请显式提供预构建运行时压缩包。

企业离线/私有源打包可通过以下环境变量配置：
- `Kantan AI_PORTABLE_PYTHON_ARCHIVE`：本地预构建运行时压缩包路径（离线 CI/CD 推荐）
- `Kantan AI_PORTABLE_PYTHON_URL`：预构建运行时压缩包下载地址
- `Kantan AI_WINDOWS_EMBED_PYTHON_VERSION` / `Kantan AI_WINDOWS_EMBED_PYTHON_URL` / `Kantan AI_WINDOWS_GET_PIP_URL`：Windows 主机构建时自动拉取源的可选覆盖项

## 架构概览

Kantan AI 采用 Electron 严格进程隔离架构，所有跨进程通信通过 IPC 完成。
...
Cowork 是 Kantan AI 的核心功能 —— 基于 Claude Agent SDK 的 AI 工作会话系统。它面向办公场景设计，能够自主完成数据分析、文档生成、信息检索等复杂任务。
...
Kantan AI 内置 16 种技能，覆盖办公、创作、自动化等多种场景，通过 `SKILLs/skills.config.json` 配置启停和排序：
...
Kantan AI 支持创建定时任务，让 Agent 按计划自动执行重复性工作。
...
Kantan AI 支持将 Agent 桥接到多种 IM 平台。在手机上通过 IM 发送消息即可远程触发桌面端的 Agent 执行任务，随时随地指挥你的个人助理。
...
| 网易小蜜蜂 | node-nim V2 SDK | [网易小蜜蜂个人数字助理](https://wp.m.163.com/163/html/bee/Kantan AI_guide/index.html) |

在设置面板中配置对应平台的 Token/密钥即可启用。配置完成后，你可以在手机 IM 中直接对 Agent 下达指令（如「帮我分析这份数据」「做一份本周工作汇报 PPT」），Agent 会在桌面端自动执行并返回结果。

## 持久记忆

Kantan AI 内置记忆系统，能够跨会话记住你的个人信息和偏好，让 Agent 越用越懂你。

### 记忆获取方式

- **自动提取** — 对话过程中，系统自动识别并记录你的个人信息（姓名、职业等）、偏好习惯（喜好的语言、格式、风格）和个人事实（养的宠物、使用的工具等），无需手动操作
- **主动告知** — 在对话中直接说「记住我喜欢用 Markdown 格式」「记下我的项目叫 Kantan AI」等，Agent 会以更高置信度存储
...
所有数据存储在本地 SQLite 数据库（`Kantan AI.sqlite`，位于用户数据目录）。
...
Kantan AI 将 OpenClaw 依赖锁定到指定的 release 版本，在 `package.json` 中声明：
...
[![Star History Chart](https://api.star-history.com/svg?repos=netease-youdao/Kantan AI&type=date&legend=top-left)](https://www.star-history.com/#netease-youdao/Kantan AI&type=date&legend=top-left)


---

感谢[网易有道]([https://github.com/netease-youdao/Lboster AI/](https://github.com/netease-youdao/lobsterai)
