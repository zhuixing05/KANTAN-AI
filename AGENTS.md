...
KANTAN AI is an Electron + React desktop application with two primary modes:
...
1. **登录：** 打开系统浏览器 → Portal 登录页 → URS 登录成功 → deep link `kantanai://auth/callback?code=<authCode>`
...
- 登录流程：`src/main/main.ts`（deep link 处理 `kantanai://` 协议）
...
- Database file: `kantanai.sqlite` in user data directory
